...
const BearsTheme = ({ ads }: { ads: any[] }) => {
  ...
  const renderItem = (item: any, i: number) => {
    ...
    return (
      <div key={i} className="flex justify-between border-b border-white/20 mb-3 text-2xl font-bold uppercase">
        <span>{title}</span>
  ...
  return (
    <div className="w-full h-screen relative bg-cover bg-center text-white font-sans overflow-hidden" 
      ...
      <h1 className="relative z-10 text-center text-7xl font-black pt-12 uppercase italic">GAME DAY <span className="text-orange-500">SPECIALS</span></h1>
      
      <div className="relative z-10 grid grid-cols-3 gap-12 p-16 mt-6">
        <div className="px-4">
          <div className="bg-orange-600 p-2 mb-6 font-black italic text-xl">KICKOFF</div>
          {getByCat('kickoff').length > 0 ? getByCat('kickoff').map((item, i) => renderItem(item, i)) : <p className="text-sm opacity-40 italic">Waiting for 'Kickoff' items...</p>}
        </div>

        <div className="bg-blue-950/50 p-6 border-t-4 border-white backdrop-blur-md">
        ...
        <div className="px-4 text-right">
          <div className="bg-orange-600 p-2 mb-6 font-black italic text-xl">DRAFT PICKS</div>
      ...
      <img src="/beer-glass.png" className="absolute bottom-6 left-8 h-56" style={{ transform: 'scaleX(-1)' }} />
      <img src="/football.png" className="absolute bottom-8 right-8 h-28" />
    </div>
  );
};
...
const TireShopTheme = ({ ads }: { ads: any[] }) => (
  <div className="h-screen bg-slate-950 text-white p-24 flex flex-col items-center justify-start overflow-hidden">
    <h1 className="text-8xl font-black mb-16 italic border-b-8 border-yellow-400 pb-6 uppercase tracking-tighter">TIRE SHOP <span className="text-yellow-400">DEALS</span></h1>
    <div className="w-full max-w-5xl space-y-8">
      {ads.map((ad, i) => (
        <div key={i} className="bg-slate-900 border-l-[12px] border-yellow-400 p-10 flex justify-between items-center shadow-2xl">
...
const App = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const filtered = data.filter((row: any) => {
          // FUZZY MATCHING: Strips spaces and converts everything to lowercase
          ...
          const cleanId = shopId.replace(/\s/g, '');
          return target === cleanId && status === 'active';
        });
        setItems(filtered);
  ...
  // If ID contains 'tire', show TireShop, else show Tony's Bar
  return shopId.includes('tire') ? <TireShopTheme ads={items} /> : <BearsTheme ads={items} />;
};
